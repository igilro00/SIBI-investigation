...
class MemoryManager:
    ...
    def get_context(self) -> str:
        ...
        if self.filters_history:
            ...
            context += (
                f"- Precio: €{last_filters.get('precio_min', 0):,} - "
                f"€{last_filters.get('precio_max', 0):,}\n"
            )
            context += (
                f"- Potencia: {last_filters.get('potencia_min', 0)} - "
                f"{last_filters.get('potencia_max', 0)} CV\n"
            )
            context += (
                f"- Autonomía mín: {last_filters.get('autonomia_min', 0)} km\n"
            )
